						  ...
						  const struct lfs_region *regions, int count)
{
	...
	relocate:
		...
		if (lfs_paircmp(oldpair, (const lfs_block_t[2])
		{
			0, 1
		}) == 0)
		{
...
int lfs_format(lfs_t *lfs, const struct lfs_config *cfg)
{
	...
	err = lfs_dir_fetch(lfs, &superdir, (const lfs_block_t[2])
	{
		0, 1
	});
	if (err)
...
int lfs_mount(lfs_t *lfs, const struct lfs_config *cfg)
{
	...
	lfs_dir_t dir;
	lfs_superblock_t superblock;
	err = lfs_dir_fetch(lfs, &dir, (const lfs_block_t[2])
	{
		0, 1
	});
	if (err && err != LFS_ERR_CORRUPT)
...
static int lfs_pred(lfs_t *lfs, const lfs_block_t dir[2], lfs_dir_t *pdir)
{
	...
	int err = lfs_dir_fetch(lfs, pdir, (const lfs_block_t[2])
	{
		0, 1
	});
	if (err)
...
static int lfs_moved(lfs_t *lfs, const void *e)
{
	...
	// skip superblock
	lfs_dir_t cwd;
	int err = lfs_dir_fetch(lfs, &cwd, (const lfs_block_t[2])
	{
		0, 1
	});
	if (err)
